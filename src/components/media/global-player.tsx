...
export function GlobalVideoPlayer() {
  ...
  useEffect(() => {
    ...
    window.navigator.mediaSession.setActionHandler('play', () => { if (playerRef.current) playerRef.current.playVideo(); setIsPlaying(true); });
    window.navigator.mediaSession.setActionHandler('pause', () => { if (playerRef.current) playerRef.current.pauseVideo(); setIsPlaying(false); });
      ...
      {isMinimized && isActive && (
        <div className="h-full flex items-center justify-between px-8 relative z-10 cursor-pointer" onClick={() => setIsFullScreen(true)}>
          ...
          <div className="flex gap-3">
            <button onClick={(e) => { e.stopPropagation(); if (playerRef.current) { if (isPlaying) playerRef.current.playVideo(); else playerRef.current.playVideo(); } setIsPlaying(!isPlaying); }} className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/10 focusable">
              ...
              <>
                ...
                {activeVideo && (
                  <>
                    ...
                    <button onClick={() => {}} className="w-12 h-12 rounded-full bg-white/5 text-emerald-400 flex items-center justify-center focusable"><Tv className="w-5 h-5" /></button>
                ...
                {activeIptv && (
                  <>
                    <button onClick={() => toggleFavoriteIptvChannel(activeIptv)} className={cn("w-12 h-12 rounded-full flex items-center justify-center focusable", isIptvStarred ? "bg-yellow-500 text-black shadow-glow" : "bg-white/5 text-white/40")}><Star className={cn("w-5 h-5", isIptvStarred && "fill-current")} /></button>
                    <div className="w-px h-8 bg-white/20 mx-1" />
