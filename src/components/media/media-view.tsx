...
export function MediaView() {
  ...
  const fetchTrendingVideos = useCallback(async () => {
    ...
    try {
      const response = await fetch('https://api.apify.com/v2/acts/plum_wobble~my-actor-7/runs/last/dataset/items?token=apify_api_LZ79CLZo893xCqB2lRnUagUf6mcj2f18MfZr&status=SUCCEEDED');
      ...
      const mapped = sortedData.map((v: any) => {
        ...
        return {
          id: vid,
          title: v.videoTitle || v.title || "فيديو بدون عنوان", // Priority to videoTitle as requested
  ...
  // Focus Management Hooks - Prioritizing Trending Videos first for the user request
  useEffect(() => {
