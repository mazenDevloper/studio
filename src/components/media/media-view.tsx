...
import { 
  ...
  Youtube, Star, Mic, Play, Clock, ChevronRight, AlertCircle, BookmarkPlus, ArrowRightLeft, ChevronUp, ChevronDown, Save
} from "lucide-react";
      ...
      <DialogContent className="bg-zinc-950/95 backdrop-blur-3xl border-white/10 text-white max-w-2xl max-h-[85vh] flex flex-col p-0 rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] outline-none">
        <div className="p-8 pb-4 border-b border-white/5 bg-black/40">
          ...
          <div className="relative mt-8">
            <Input ref={inputRef} placeholder={type === 'channel' ? "ابحث عن قنوات يوتيوب..." : "ابحث عن قراء ومبتهلين..."} value={query} onChange={(e) => handleSearch(e.target.value)} className="h-16 bg-white/5 border-white/10 rounded-2xl pr-14 text-xl focusable text-right text-white" autoFocus />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pt-4 space-y-3">
          {results.map((item, i) => (
            <div key={item.channelid} onClick={() => { onAdd(item); onOpenChange(false); }} className="flex items-center gap-5 p-5 rounded-[2rem] bg-white/5 hover:bg-white/10 transition-all cursor-pointer border-4 border-transparent group focusable outline-none" tabIndex={0}>
...
export function MediaView() {
  ...
  const { 
    ...
    fetchPriorityData, lastLiveUpdate, setLastLiveUpdate, isReorderMode, toggleReorderMode, reorderChannel,
    saveChannelsReorder, pickedUpId, setPickedUpId, reorderChannelTo
  } = useMediaStore();
  ...
  const [yesterdayMatches, setYesterdayMatches] = useState<YouTubeVideo[]>([]);
  ...
  const handleSurahClick = (name: string) => { setSelectedSurah(name); const finalQuery = updateDynamicSearch(selectedReciter, name, selectedStyle); performSearch(finalQuery); };

  const fetchFeeds = useCallback(async () => {
    if (!favoriteChannels.length) return;
    try {
      const starred = favoriteChannels.filter(c => c.starred);
      const allStarredPromises = starred.map(c => fetchChannelVideos(c.channelid, 5));
      ...
      searchYouTubeVideos("ملخص مباريات أمس", 15).then(v => setYesterdayMatches(v));
      searchYouTubeVideos("كلمات الطفل الأولى ريان بالعربي", 30).then(v => setKidsVideos(v));
    } catch (e) {}
  }, [favoriteChannels]);
  ...
  const interleavedKids = useMemo(() => {
    const ryan = kidsVideos.filter(v => v.channelTitle?.includes("ريان") || v.title.includes("ريان"));
    const kids = kidsVideos.filter(v => !ryan.includes(v));
    const result: YouTubeVideo[] = [];
    const max = Math.max(ryan.length, kids.length);
    for (let i = 0; i < max; i++) {
      if (ryan[i]) result.push(ryan[i]);
            ...
            {favoriteChannels.map((ch, idx) => (
              <div key={ch.channelid} onClick={() => { if(!isReorderMode) { setSearchResults([]); setSelectedChannel(ch); } }} className={cn("flex flex-row-reverse items-center p-3 rounded-xl w-[94%] mx-auto gap-3 cursor-pointer focusable shrink-0 border-2", selectedChannel?.channelid === ch.channelid ? "bg-primary text-white" : "hover:bg-white/5 text-white/60", "border-transparent relative group")} tabIndex={0} data-nav-id={`subs-${idx + 1}`} data-type="channel" data-id={ch.channelid}>
                ...
                {isReorderMode && !isSidebarShrinked && (
                  <div className="flex flex-col gap-1 absolute left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); reorderChannel(ch.channelid, 'prev'); }} className="w-6 h-6 rounded bg-white/10 flex items-center justify-center"><ChevronUp className="w-3 h-3 text-white" /></button>
                    <button onClick={(e) => { e.stopPropagation(); reorderChannel(ch.channelid, 'next'); }} className="w-6 h-6 rounded bg-white/10 flex items-center justify-center"><ChevronDown className="w-3 h-3 text-white" /></button>
                  </div>
                )}
              </div>
            ))}
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar relative pt-0 pb-40 px-10 no-scrollbar" style={{ direction: 'rtl' }}>
        {!showIsolatedView ? (
          <>
            ...
            <section className={rowWrapperClass}><div className={horizontalListClass}><button onClick={() => { setSelectedStyle(null); updateDynamicSearch(selectedReciter, selectedSurah, null); }} className={cn(itemScaleClass, "px-8 py-4 rounded-full font-black text-sm focusable border-2 text-white", !selectedStyle ? "bg-primary border-primary/40" : "bg-white/5 border-transparent")} tabIndex={0}>الكل</button>{READING_STYLES.map((style, i) => (<button key={style} data-nav-id={`style-${i}`} onClick={() => handleStyleClick(style)} className={cn(itemScaleClass, "px-8 py-4 rounded-full font-black text-sm focusable border-2 text-white", selectedStyle === style ? "bg-primary border-primary/40 shadow-glow" : "bg-white/5 border-white/5")} tabIndex={0}>{style}</button>))}</div></section>
            ...
            <section className={rowWrapperClass}><div className={horizontalListClass}>{yesterdayMatches.map((v, idx) => (<div key={v.id + idx} onClick={() => setActiveVideo(v, yesterdayMatches)} className={cn(itemScaleClass, "w-72 group relative overflow-hidden bg-zinc-900/80 border-b-4 border-blue-600/40 rounded-[1.8rem] focusable cursor-pointer shadow-2xl")} tabIndex={0}><div className="aspect-video relative overflow-hidden"><img src={v.thumbnail} className="w-full h-full object-cover" alt="" /></div><div className="p-4 text-right"><h3 className="font-bold text-xs truncate text-white">{v.title}</h3></div></div>))}</div></section>
                ...
                {activeGridVideos.map((v, i) => {
                  ...
                  return (
                    <Card key={v.id || i} className="group bg-zinc-900/40 border-none rounded-[2.8rem] cursor-pointer focusable overflow-hidden shadow-2xl aspect-[16/10] relative" tabIndex={0} onClick={() => setActiveVideo(v, activeGridVideos)}>
                      <div className="w-full h-full relative"><img src={v.thumbnail} className="w-full h-full object-cover opacity-70 group-hover:opacity-100" alt="" /><div className="absolute inset-0 bg-gradient-to-tr from-black/95 via-black/20 to-transparent flex flex-col justify-end items-end p-4 text-right"><div className="absolute top-4 left-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all"><button onClick={(e) => { e.stopPropagation(); toggleSaveVideo(v); }} className={cn("w-10 h-10 rounded-full backdrop-blur-xl border border-white/20 flex items-center justify-center transition-all", isSaved ? "bg-accent text-black shadow-glow" : "bg-white/10 text-white/60")}><BookmarkPlus className="w-5 h-5" /></button></div>{progress > 0 && <div className="h-0.5 w-full bg-white/10 rounded-full overflow-hidden mb-1"><div className="h-full bg-accent" style={{ width: `${Math.min(100, (progress / 3600) * 100)}%` }} /></div>}<div className="flex items-center justify-between w-full opacity-40"><div className="flex items-center gap-1.5"><Clock className="w-2.5 h-2.5 text-white" /><span className="text-[8px] font-black text-white uppercase tracking-widest">{v.duration || "FEED"}</span></div>{v.isLive && <div className="flex items-center gap-1"><div className="w-1 h-1 rounded-full bg-red-600 animate-pulse" /><span className="text-[8px] font-black text-red-500 uppercase tracking-widest">LIVE</span></div>}</div></div></div>
                    </Card>
                  );
                })}
              </div>
