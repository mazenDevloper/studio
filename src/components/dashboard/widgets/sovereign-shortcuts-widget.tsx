...
import { 
  Sparkles, Youtube, Search, Trophy, RefreshCw, Activity, BookOpen, Loader2, MonitorPlay, Edit3, Save, Radio 
} from "lucide-react";
...
export function SovereignShortcutsWidget() {
  ...
  const { 
    ...
    syncMasterBin, mapSettings, updateMapSettings, fetchSpecificBin
  } = useMediaStore();
  ...
  const executeSpiritualPulse = useCallback(async () => {
    ...
    try {
      ...
      if (liveVideo) { setActiveVideo(liveVideo); } 
      else { setActiveQuranUrl("https://quran.com/ar/radio?autoplay=1"); router.push("/quran"); }
    } catch (e) {
      toast({ variant: "destructive", title: "خطأ", description: "فشل استدعاء البث" });
    } finally { setIsQuranProcessing(false); }
  ...
  const executeOmanLive = useCallback(async () => {
    ...
    try {
      setActiveIptv({ stream_id: "oman-tv-live", name: "قناة عمان مباشر", stream_icon: OMAN_TV_AVATAR, category_id: "direct", url: mapSettings.omanUrl || DEFAULT_OMAN_URL, type: 'web' });
    } catch (e) {
      toast({ variant: "destructive", title: "خطأ", description: "فشل استدعاء بث عمان" });
    } finally { setIsOmanProcessing(false); }
  ...
  const handleStartEdit = (id: string, currentUrl: string) => { setEditingId(id); setTempUrl(currentUrl || (id === 'oman-live' ? DEFAULT_OMAN_URL : "")); };
  ...
  ], [lastPlayedVideo, isSystemRefreshing, isQuranProcessing, isOmanProcessing, executeSpiritualPulse, executeOmanLive, executeSystemOptimizer, router, setActiveVideo, setActiveIptv, mapSettings, setActiveQuranUrl]);

  return (
    <div className="grid grid-cols-6 gap-4 p-8 h-full items-center">
      {commands.map((cmd, idx) => {
        const Icon = cmd.icon; const isEditing = editingId === cmd.id;
        return (
          <div key={cmd.id} className="relative h-48 group">
             <button onClick={isEditing ? undefined : cmd.action} disabled={cmd.disabled || cmd.isLoading} className={cn("w-full h-full rounded-[3.5rem] overflow-hidden flex flex-col p-8 text-right dir-rtl focusable transition-all active:scale-95 border-2", (cmd.disabled || cmd.isLoading) ? "opacity-40 grayscale cursor-not-allowed" : "cursor-pointer hover:border-white/20", `bg-gradient-to-br ${cmd.gradient} border-white/5 backdrop-blur-3xl shadow-2xl`)} tabIndex={0} data-nav-id={`shortcut-item-${idx}`}>
              <div className="relative z-10 flex flex-col h-full justify-between w-full">
                <div className={cn("w-16 h-16 rounded-[2rem] bg-black/40 backdrop-blur-3xl flex items-center justify-center border border-white/10 shadow-2xl group-hover:scale-110 transition-transform relative overflow-hidden", cmd.isLoading && "animate-spin")}>{cmd.avatar ? (<img src={cmd.avatar} className="w-full h-full object-cover rounded-[2rem]" alt="" />) : cmd.isLoading ? (<Loader2 className="w-8 h-8 text-white" />) : (<Icon className={cn("w-9 h-9 text-white")} />)}</div>
                {isEditing ? (<div className="mt-auto space-y-2 w-full animate-in zoom-in-95 duration-200"><Input value={tempUrl} onChange={(e) => setTempUrl(e.target.value)} className="h-8 bg-black/60 border-white/20 text-[8px] font-bold text-white px-2" placeholder="URL..." /><button onClick={(e) => { e.stopPropagation(); handleSaveEdit(); }} className="w-full h-6 bg-emerald-500 rounded-lg flex items-center justify-center text-black"><Save className="w-3 h-3" /></button></div>) : (<div className="flex flex-col gap-1 mt-auto"><span className="text-xl font-black text-white tracking-tighter truncate leading-tight">{cmd.label}</span><span className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em] truncate">{cmd.sublabel}</span></div>)}
              </div>
            </button>
            {(cmd.editable && !isEditing) && (<button onClick={(e) => { e.stopPropagation(); handleStartEdit(cmd.id, cmd.url || ""); }} className="absolute top-4 left-4 w-8 h-8 rounded-full bg-white/10 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity focusable z-20"><Edit3 className="w-4 h-4" /></button>)}
          </div>
        );
      })}
    </div>
