...
export function RemindersWidget() {
  ...
  const activeReminders = useMemo(() => {
    ...
    return reminders.filter(reminder => {
      const { startHour, endHour } = reminder;
  ...
  }, [reminders, currentHour]);

  return (
    <div className="h-full bg-zinc-900/40 backdrop-blur-3xl rounded-[2.5rem] p-6 border border-white/5 flex flex-col gap-4 shadow-2xl overflow-hidden transition-all duration-500">
      <div className="flex items-center justify-between mb-1">
         <div className="flex flex-col">
           <h2 className="text-sm font-bold font-headline text-white/40 uppercase tracking-widest">التذكيرات النشطة</h2>
           <span className="text-[10px] text-accent font-bold uppercase tracking-tighter opacity-60">حسب وقتك الحالي</span>
         </div>
        ...
        <div className="flex flex-col gap-3">
          {activeReminders.length === 0 ? (
            <div className="py-8 text-center opacity-20">
              <p className="text-xs font-bold uppercase tracking-widest">لا توجد تذكيرات نشطة حالياً</p>
            </div>
          ) : (
