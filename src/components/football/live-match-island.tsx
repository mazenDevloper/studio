        ...
        if (isActive) {
          ...
          return (
            <div 
              key={match.id} 
              onClick={() => handleIslandClick(idx)}
              className="pointer-events-auto w-16 h-16 rounded-full liquid-glass border border-white/20 flex flex-col items-center justify-center p-0 shadow-2xl cursor-pointer hover:scale-110 active:scale-90 transition-all focusable outline-none relative overflow-hidden"
            >
               ...
               <div className="flex flex-col items-center gap-0 relative z-10 w-full h-full justify-center">
                  ...
                  <div className="w-full flex items-center justify-center leading-none mt-0.5">
                    {isLive && (
                      <span className="text-base font-black text-primary tabular-nums tracking-tighter">
                        {match.score?.home}-{match.score?.away}
                      </span>
                    )}
                    {!isLive && (
                      <span className="text-[10px] font-black text-white/90 tabular-nums leading-none text-center">
                        {match.startTime}
                      </span>
                    )}
                  </div>
