...
export function LiveMatchIsland() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      // ترتيب الأولويات: مباشر مفضل > مباشر عام > قادم مفضل > قادم عام
      const favLive = matches.filter(m => m.status === 'live' && isFavoriteMatch(m)).sort((a,b) => (b.minute || 0) - (a.minute || 0));
      const genLive = matches.filter(m => m.status === 'live' && !isFavoriteMatch(m)).sort((a,b) => (b.minute || 0) - (a.minute || 0));
      const favUpcoming = matches.filter(m => m.status === 'upcoming' && isFavoriteMatch(m)).sort((a,b) => new Date(a.date || "").getTime() - new Date(b.date || "").getTime());
      const genUpcoming = matches.filter(m => m.status === 'upcoming' && !isFavoriteMatch(m)).sort((a,b) => new Date(a.date || "").getTime() - new Date(b.date || "").getTime());

      const prioritized = [...favLive, ...genLive, ...favUpcoming, ...genUpcoming];

      if (prioritized.length > 0) {
        setPrimaryMatch(prioritized[0]);
        setNextMatches(prioritized.slice(1, 3));
      } else {
        setPrimaryMatch(null);
        setNextMatches([]);
      }
  ...
  const isLive = primaryMatch.status === 'live';
  ...
  return (
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 pointer-events-none">
            ...
            {!isExpanded ? (
              <div className="flex items-center justify-between w-full animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="flex items-center gap-4">
                  <img src={primaryMatch.homeLogo} alt="" className="w-9 h-9 object-contain drop-shadow-md" />
                  <div className={cn(
                    ...
                    <span className={cn(
                      ...
                      {isLive ? `${primaryMatch.score?.home} - ${primaryMatch.score?.away}` : primaryMatch.startTime}
                    </span>
                  </div>
                  <img src={primaryMatch.awayLogo} alt="" className="w-9 h-9 object-contain drop-shadow-md" />
                </div>
                ...
                <div className="flex items-center gap-3 pr-2">
                  {isLive ? (
                    <>
                      ...
                      <span className="text-sm font-black text-accent tabular-nums">{primaryMatch.minute}'</span>
                    </>
                  ) : (
                    <>
              ...
              <div className="flex items-center justify-between w-full animate-in fade-in zoom-in-95 duration-700">
                <div className="flex flex-col items-center gap-3 w-32 group">
                  <img src={primaryMatch.homeLogo} alt="" className="w-20 h-20 object-contain transition-transform group-hover:scale-110" />
                  <span className="text-[10px] font-black text-white/90 truncate w-full text-center uppercase tracking-tighter">{primaryMatch.homeTeam}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="flex items-center gap-8">
                    {isLive ? (
                      <>
                        <span className="text-7xl font-black text-white drop-shadow-[0_0_40px_rgba(255,255,255,0.1)] tabular-nums">{primaryMatch.score?.home}</span>
                        <div className="flex flex-col items-center gap-2">
                          <span className="text-3xl font-black text-primary tracking-tighter animate-pulse">{primaryMatch.minute}'</span>
                        </div>
                        <span className="text-7xl font-black text-white drop-shadow-[0_0_40px_rgba(255,255,255,0.1)] tabular-nums">{primaryMatch.score?.away}</span>
                      </>
                    ) : (
                  ...
                  <div className="flex flex-col items-center gap-1">
                     <div className="text-[9px] font-bold text-white/40 uppercase tracking-[0.3em] flex items-center gap-2 text-center">
                      ...
                      <span className="truncate max-w-[220px]">{primaryMatch.league}</span>
                    </div>
                  </div>
                </div>

                <div className="flex flex-col items-center gap-3 w-32 group">
                  <img src={primaryMatch.awayLogo} alt="" className="w-20 h-20 object-contain transition-transform group-hover:scale-110" />
                  <span className="text-[10px] font-black text-white/90 truncate w-full text-center uppercase tracking-tighter">{primaryMatch.awayTeam}</span>
                </div>
              </div>
            )}
          </div>
      ...
      {/* الجزر المصغرة (Next 2 Islands) */}
      {!isExpanded && nextMatches.map((m, idx) => (
        <div 
          key={m.id} 
          className="pointer-events-auto w-16 h-16 rounded-full bg-black/95 backdrop-blur-3xl border border-white/20 flex flex-col items-center justify-center p-2 shadow-2xl ring-4 ring-white/5 animate-in fade-in slide-in-from-right-4 duration-1000"
          style={{ transitionDelay: `${(idx + 1) * 200}ms` }}
        >
           <div className="flex flex-col gap-1 items-center scale-90">
              <img src={m.homeLogo} alt="" className="w-6 h-6 object-contain drop-shadow-sm" />
              <div className="h-px w-4 bg-white/10" />
              <img src={m.awayLogo} alt="" className="w-6 h-6 object-contain drop-shadow-sm" />
           </div>
        </div>
      ))}
    </div>
