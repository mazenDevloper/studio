...
export function FootballView() {
  ...
  const loadMatches = async (view: string) => {
    setLoading(true);
    try {
      const result = await fetchFootballData(view === 'live' ? 'live' : 'today');
      setMatches(result);
      setError(null);
    } catch (err: any) {
  ...
  const filteredAndSortedMatches = useMemo(() => {
    ...
    if (activeTab === "live") {
      ...
      result = result.filter(m => isFavTeam(m.homeTeamId) || isFavTeam(m.awayTeamId) || isFavLeague(m.competition?.id));
    }
  ...
  const renderMatchCard = (match: any, idx: number) => {
    const isFav = isFavTeam(match.homeTeamId) || isFavTeam(match.awayTeamId);
        ...
        <CardContent className="p-5">
          <div className="flex flex-col gap-5">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              <span className="text-[10px] font-black uppercase tracking-tight text-white/70 truncate max-w-[70%]">
                {match.league}
              </span>
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-col items-center flex-1 gap-2">
                <div className={cn("h-14 w-14 rounded-2xl p-2.5 flex items-center justify-center border", isFavTeam(match.homeTeamId) ? "bg-primary/20 border-primary" : "bg-white/5 border-white/5")}>
                  <img src={match.homeLogo} alt="" className="h-full w-full object-contain" />
                </div>
                <span className={cn("text-xs font-black text-center line-clamp-2 min-h-[32px]", isFavTeam(match.homeTeamId) ? "text-primary" : "text-white")}>
                  {match.homeTeam}
                </span>
              </div>

              {/* النتيجة والوقت بالحجم الكبير جداً كما طلب المستخدم */}
              <div className="flex flex-col items-center justify-center min-w-[140px] gap-1">
                {match.status === "upcoming" ? (
                  ...
                  <div className="text-5xl font-black tabular-nums tracking-tighter drop-shadow-xl text-white">
              ...
              <div className="flex flex-col items-center flex-1 gap-2">
                <div className={cn("h-14 w-14 rounded-2xl p-2.5 flex items-center justify-center border", isFavTeam(match.awayTeamId) ? "bg-primary/20 border-primary" : "bg-white/5 border-white/5")}>
                  <img src={match.awayLogo} alt="" className="h-full w-full object-contain" />
                </div>
                <span className={cn("text-xs font-black text-center line-clamp-2 min-h-[32px]", isFavTeam(match.awayTeamId) ? "text-primary" : "text-white")}>
                  {match.awayTeam}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 py-3 border-t border-white/5 mt-2">
              <div className="flex items-center gap-1.5">
                ...
                <div className="flex flex-col">
                  <span className="text-[8px] text-muted-foreground font-bold uppercase">القناة</span>
  ...
  return (
    <div className="p-8 space-y-8 pb-32">
      <header className="flex items-center justify-between">
        ...
        <Button variant="outline" onClick={() => loadMatches(activeTab)} disabled={loading} className="rounded-full bg-white/5 border-white/10 text-white">
      ...
      <Tabs defaultValue="today" onValueChange={setActiveTab} className="w-full">
        <div className="flex justify-center mb-8">
          <TabsList className="bg-white/5 border border-white/10 p-1.5 rounded-[2rem] h-16 w-full max-w-xl shadow-2xl backdrop-blur-3xl">
            ...
            <TabsTrigger value="live" className="flex-1 rounded-[1.5rem] font-black text-xs data-[state=active]:bg-red-600 flex items-center gap-2">
              <Activity className="h-4 w-4" /> المباشرة
            </TabsTrigger>
            <TabsTrigger value="favorites" className="flex-1 rounded-[1.5rem] font-black text-xs data-[state=active]:bg-yellow-500 data-[state=active]:text-black flex items-center gap-2">
        ...
        <TabsContent value={activeTab}>
          {loading && matches.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 gap-6">
              ...
              <p className="text-muted-foreground font-black animate-pulse">جاري جلب البيانات...</p>
            </div>
            ...
            <div className="text-center py-24 flex flex-col items-center gap-6 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
              ...
              <h3 className="text-2xl font-black text-white">لا توجد مباريات</h3>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-8">
