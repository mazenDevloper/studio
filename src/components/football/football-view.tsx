...
import { Loader2, Trophy, RefreshCw, Bell, BellRing } from "lucide-react";
...
export function FootballView() {
  ...
  const { belledMatchIds, toggleBelledMatch, setActiveIptv, dockSide } = useMediaStore();
  const isDockLeft = dockSide === 'left';

  const loadMatches = async () => {
    setLoading(true);
    try {
      const result = await fetchFootballData(activeTab as any);
      setMatches(result || []);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={cn("p-8 space-y-8 pb-32", isDockLeft ? "text-right dir-rtl" : "text-left dir-ltr")}>
      <header className="flex items-center justify-between">
        ...
        <Button variant="outline" onClick={loadMatches} disabled={loading} className="rounded-full bg-white/5 border-white/10 text-white focusable h-12 px-8" data-nav-id="football-refresh-btn">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-white/5 border border-white/10 p-1 rounded-full h-16 w-full max-w-2xl backdrop-blur-3xl shadow-2xl">
          <TabsTrigger value="yesterday" className="flex-1 rounded-full font-black text-xs focusable relative" data-nav-id="football-tab-yesterday">
            أمس
          </TabsTrigger>
          <TabsTrigger value="today" className="flex-1 rounded-full font-black text-xs focusable relative" data-nav-id="football-tab-today">
            اليوم
          </TabsTrigger>
          <TabsTrigger value="tomorrow" className="flex-1 rounded-full font-black text-xs focusable relative" data-nav-id="football-tab-tomorrow">
            غداً
          </TabsTrigger>
          <TabsTrigger value="live" className="flex-1 rounded-full font-black text-xs data-[state=active]:bg-red-600 focusable relative" data-nav-id="football-tab-live">
            مباشر
          </TabsTrigger>
        </TabsList>
      </Tabs>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {matches.map((match, idx) => (
              <Card 
                ...
                className="relative bg-white/5 border-white/5 rounded-[2.5rem] overflow-hidden focusable group transition-all hover:bg-white/10 h-56 grid-item"
                tabIndex={0}
                ...
                <CardContent className="p-6 h-full flex flex-col justify-between">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex flex-col items-center flex-1 gap-2">
                      <div className="w-16 h-16 rounded-2xl bg-black/20 p-2 flex items-center justify-center border border-white/5"><img src={match.homeLogo} className="w-full h-full object-contain" alt="" /></div>
                    ...
                    <div className="flex flex-col items-center justify-center min-w-[80px]">
                      <div className="text-3xl font-black text-white tabular-nums tracking-tighter">{match.status === 'live' ? `${match.score.away}-${match.score.home}` : match.startTime}</div>
                      {match.status === 'live' && <span className="text-primary text-[10px] font-black animate-pulse">{match.minute}'</span>}
                    </div>
                    <div className="flex flex-col items-center flex-1 gap-2">
                      <div className="w-16 h-16 rounded-2xl bg-black/20 p-2 flex items-center justify-center border border-white/5"><img src={match.awayLogo} className="w-full h-full object-contain" alt="" /></div>
